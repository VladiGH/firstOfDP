...
void fastestWay(int a[][3], int t[][2], int e[2], int x[2], int n){
    ...
    for (int j = 1; j < n; j++){
        if((f1[j -1] + a[0][j]) <= (f2[j-1]+ t[1][j-1]+ a[0][j]) ){
            ...
            //camino1[j] = to_string(f2[j-1]) +"-"+ to_string(t[1][j-1])+"-"+to_string(a[0][j]) + "|";
            camino1[j] = to_string(j-1)+" "+"t1"+to_string(j-1)+" "+"a0"+to_string(j)+" ";
        }

        if((f2[j-1]+a[1][j]) <= (f1[j-1]+t[0][j-1]+a[1][j])){
            f2[j] = f2[j-1]+a[1][j];
            //camino2[j] = to_string(f2[j-1]) +"-"+ to_string(a[1][j])+ "|";
            camino2[j] = to_string(j-1)+" "+"a1"+to_string(j)+ " ";
        
        }
        else{
            f2[j] = f1[j-1] + t[0][j-1]+a[1][j];
            //camino2[j] = to_string(f1[j-1]) +"-"+ to_string(t[0][j-1])+"-"+to_string(a[1][j]) + "|";
